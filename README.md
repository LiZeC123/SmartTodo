...
直接在标题栏输入代办事项的标题即可创建一个基础的代办事项. 创建时可以选择任务的重要程度, 默认为`普通任务`, 可修改为`低优任务`或者`高优任务`. 不同任务具有如下的区别
...
将每周日视为一周的第一天, 每周六视为一周的最后一天. 基于以上机制可确保所有任务都会设置一个默认的截止时间, 并且可以在周六的时候统一清理本周还未完成的任务.
