...
便签是一种特殊的待办事项, 在创建代办事项时, 如果名称以`计划`, `规划`, `事项`, `分析`, `笔记`结尾, 则会询问用户是否需要创建一个便签类型的代办事项.
...
助理模式支持与待办事项系统交互, 具体包括

1. 根据用户要求创建待办事项，支持设置名称、截止日期、优先级
2. 查询今日待办事项和已经逾期的待办事项
...
在生产环境下，系统每日自动执行记忆压缩任务：遍历所有用户的所有活跃角色，对符合压缩条件的角色执行长期记忆更新。非生产环境（`ENV` 不为 `PROD`）不执行此任务以避免不必要的 LLM 调用费用。
