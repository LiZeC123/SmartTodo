...
MemoryPolicyDict = {
    ...
    "S1": MemoryPolicy(
        enable_role_setting=True, enable_preference=False, max_topic_num=0, max_diary_num=2, raw_content_size=9 * KB
    ),
    # 角色设定优先, 类似于S1, 但对于对话连续性要求更低
    "S2": MemoryPolicy(
        enable_role_setting=True, enable_preference=False, max_topic_num=0, max_diary_num=1, raw_content_size=6 * KB
    ),
    ...
    "D2": MemoryPolicy(
        enable_role_setting=False, enable_preference=False, max_topic_num=0, max_diary_num=3, raw_content_size=6 * KB
    ),
    # 话题优先, 适合人设几乎不会变化, 且历史对话没有太大关联, 经常讨论不同话题的场景
    "T1": MemoryPolicy(
        enable_role_setting=False, enable_preference=False, max_topic_num=12, max_diary_num=2, raw_content_size=5 * KB
    ),
}

DefaultMemoryPolicy = MemoryPolicy(
    enable_role_setting=False, enable_preference=False, max_topic_num=0, max_diary_num=0, raw_content_size=6 * KB
)
